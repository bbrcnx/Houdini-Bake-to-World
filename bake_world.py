...
#v1.2 2018_06_30 Author:wang xiaowei  Email:wangxiaowei@ckyhvfx.com
...
for node in sel_nodes:
    ...
    if t != None and r != None and s != None:
...
for old_node in convert_nodes:
    ...
    new_node.parmTuple('s').deleteAllKeyframes()    #delete all the scale keyframes
    ...
    chopobject.setParms({"targetpath":node_path,"compute": 7,"rOrd":rot_order,"start":start,"end":end,"units":0,"export":new_node.path()})
    ...
    for s in new_node.parmTuple("./s"):
        s.keyframesRefit(1, 0, 1, 1, 1, 1.0, 0, 1, start, end, hou.parmBakeChop.KeepExportFlag)
